...
describe('Order Execution - Completed Order', () => {
    ...
    it('Unsuccessfull Delivery Order', async()=>{
        ...
        const orderTab = await $('//android.widget.TextView[@text="Orders"]');
        const deliverTab = await $('//android.widget.TextView[@text="Deliver"]');
        const firstDeliverOrder = $('//android.widget.TextView[@text="  AUTOMATION TEST M03"]'); 
        ...
        const unSuccessBtn1 = await $('~, Unsuccessful');
        const reasonText = await $('//android.widget.TextView[@text="Reason *"]');
        const reasonField = await $('//android.widget.EditText[@text="Enter a reason..."]');
        const unSuccessBtn2 = await $('~Confirm Unsuccessful');
        ...
        await expect(fieldName).toHaveText("Tata");
        await expect(unSuccessBtn1).toBeDisplayed();
        await unSuccessBtn1.click();
        await expect(reasonText).toBeDisplayed();
        await expect(reasonField).toBeDisplayed();
        await reasonField.setValue("test reason");
        await expect(unSuccessBtn2).toBeDisplayed();
        await unSuccessBtn2.click();
    });
